...
class debugger():
    ...
    def load(self, path_to_exe):
        ...
        # The following two options allow the started process to be shown as a separate window.  This also
        # illustrates how different settings in the STARTUPINFO struct can affect the debugee.
        startupinfo.dwFlags = 0x1
        startupinfo.dwShowWindow = 0x0

        # We then initialize the cb variable in the STARTUPINFFO struct which is just the size of the struct ifself
    ...
    def bp_set(self, address):
        ...
        if not self.breakpoints.has_key(address):
            try:
                origional_byte = self.read_process_memory(address, 1)

                # write INIT3 opcode
                self.write_process_memory(address, "\xCC")
                self.breakpoints[address] = (address, origional_byte)
            except:
                return False
    ...
    def bp_set_hw(self, address, length, condition):
        ...
        self.h_thread = self.open_thread(thread_id=thread_id)
